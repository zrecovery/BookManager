...
let mockSavedArticles  = async () => {
    ...
    return await Promise.all(mockArticles.map(async article => {
...
test("能否保存文章", async () => {
    let article = new Article("test title", "test book", "test author", "test content\ntest line2", 1.0)
...
test("能否读取文章",async () => {
    ...
    exceptedArticles.map(async(testArticle) => expect(await mockArticleStore.get(testArticle.id)).toEqual(testArticle))
})
